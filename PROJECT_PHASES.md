...
- **Q5 (revised): Engineer animal-keyword flags from `Other Animal Sightings`; final keyword set delegated to the preprocessing teammate.**
  ...
  - Confirmed flags: `animals_humans_present` (keyword `"human"`) and `animals_data_missing` for the 32 null rows in `hectare.csv`.
  - **Open**: which species keywords to flag (e.g. `"dog"` is a likely candidate, but the final list depends on the free-text vocabulary). Delegated to the preprocessing teammate, who will inspect the full dataset and propose the final keyword list before the modelling table is finalised.
...
- Animal flags (engineered): `animals_humans_present`, plus additional species flags TBD by the preprocessing teammate after free-text inspection, plus `animals_data_missing` for null rows.
...
- **Q3 (closed): Single 80/20 hold-out + inner 5-fold CV.**
  - Steps:
    ...
    2. On the 80% training portion: 5-fold `StratifiedGroupKFold` (same stratify + group rules) inside `GridSearchCV` to choose model hyperparameters. Inner-CV scorer = `average_precision` (threshold-independent, minority-class-sensitive; see Phase 4 Q3 for the AP-vs-ROC-AUC rationale).
